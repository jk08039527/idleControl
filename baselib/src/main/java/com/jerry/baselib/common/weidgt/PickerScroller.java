...
public class PickerScroller {
    ...
    private static float sDecelerationRate = (float) (Math.log(0.75) / Math.log(0.9));
    private static float sAlpha = 800; // pixels / seconds
    private static float sStartTension = 0.4f; // Tension at start: (0.4 * total T, 1.0 * Distance)
    private static float sEndTension = 1.0f - sStartTension;
    ...
    static {
        ...
        for (int i = 0; i <= NB_SAMPLES; i++) {
            ...
            while (true) {
                ...
                coef = 3.0f * x * (1.0f - x);
                tx = coef * ((1.0f - x) * sStartTension + x * sEndTension) + x * x * x;
     ...
     */
    public PickerScroller(Context context) {
        this(context, null);
    }

    /**
     * Create a Scroller with the specified interpolator. If the interpolator is null, the default (viscous) interpolator will be used. "Flywheel"
     * behavior will be in effect for apps targeting Honeycomb or newer.
     */
    public PickerScroller(Context context, Interpolator interpolator) {
        this(context, interpolator, true);
    }

    /**
     * Create a Scroller with the specified interpolator. If the interpolator is null, the default (viscous) interpolator will be used. Specify
     * whether or not to support progressive "flywheel" behavior in flinging.
     */
    public PickerScroller(Context context, Interpolator interpolator, boolean flywheel) {
        mFinished = true;
    ...
    public void fling(int startX, int startY, int velocityX, int velocityY, int minX, int maxX, int minY, int maxY) {
        ...
        mVelocity = velocity;
        final double l = Math.log(sStartTension * velocity / sAlpha);
        mDuration = (int) (1000.0 * Math.exp(l / (sDecelerationRate - 1.0)));
        mStartTime = AnimationUtils.currentAnimationTimeMillis();
        mStartX = startX;
        mStartY = startY;
        ...
        int totalDistance = (int) (sAlpha * Math.exp(sDecelerationRate / (sDecelerationRate - 1.0) * l));
    ...
    /**
     * Stops the animation. Contrary to {@link #forceFinished(boolean)}, aborting the animating cause the scroller to move to the final x and y
     * position
     *
    ...
    /**
     * Extend the scroll animation. This allows a running animation to scroll further and longer, when used with {@link #setFinalX(int)} or {@link
     * #setFinalY(int)}.
     *
